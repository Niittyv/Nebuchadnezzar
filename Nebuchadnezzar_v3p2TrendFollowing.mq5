...
//| - Counter signal removed. Sl, Tp and timeout only. (APPLIED)     |
...
string GetTrendFollowingSignal()
{
  ...
  if(LastCandleColor == "bull")
  {
    ...
    int HighestIndexClose = iHighest(Symbol(),Timeframe,MODE_CLOSE,BufferCandles,2);
    double HighestPreviousClose = NormalizeDouble(iClose(Symbol(),Timeframe,HighestIndexClose), 10);
  ...
  else if(LastCandleColor == "bear")
  {
    ...
    int LowestIndexClose = iLowest(Symbol(),Timeframe,MODE_CLOSE,BufferCandles,2);
    double LowestPreviousClose = NormalizeDouble(iClose(Symbol(),Timeframe,LowestIndexClose), 10);
...
ulong ProcessTradeOpen(ENUM_ORDER_TYPE OrderType)
{
  ...
  else if(OrderType == ORDER_TYPE_SELL && ExistingPosition == "no positions")
  {
    ...
    Print("Can't open position -- Previous trade is still open ja arin 3cm millikiituri on niin tylppä ja käyrä että sillä ei panna edes hevosta perseeseen");
    return(Ticket);
  }
